...
const baseDirectory = '/Users/oodemi/Code/photogImages';
...
const generateBlurredImageBase64 = async (filePath) => {
...
const updateImageDimensionsAndGenerateBlurred = async (images) => {
  for (const image of images) {
    if (image.src) {
      const imagePath = image.src.split('/').slice(-2).join('/');
      const fullPath = path.join(baseDirectory, imagePath);
      if (fs.existsSync(fullPath)) {
        const dimensions = await getDimensions(fullPath);
        image.width = dimensions.width;
        image.height = dimensions.height;

        image.lowResSrc = await generateBlurredImageBase64(fullPath);
      }
    }
  }
  return images;
...
const main = async () => {
  const updatedImages = await updateImageDimensionsAndGenerateBlurred(images);
  const outputFilePath = path.join(__dirname, 'imagesMetadata.js');

  let outputContent = 'const images = [\n';
  ...
  updatedImages.forEach((image, index) => {
    ...
    if (index > 0 && !images[index - 1].src) {
      imageString = `  /*\n${imageString}  */\n`;
    }

    outputContent += imageString;
  });

  outputContent += '];\n';

  fs.writeFileSync(outputFilePath, outputContent, 'utf8');
  console.log(`Updated image dimensions and placeholders saved to ${outputFilePath}`);
};
